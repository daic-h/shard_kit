...
RSpec.describe ShardingKit::Model, clean_all: true do
  describe ".using" do
    it "should allow to pass a string as the shard name to a AR subclass" do
      user = User.using(:shard_1).create!(name: "Yuki")

      expect(User.using(:shard_1).where(name: "Yuki").take).to eq(user)
    end

    it "should allow to pass a string as the shard name to a block" do
      user = User.using(:shard_1) do
    ...
    it "should allow selecting the shards on scope" do
      User.using(:shard_1).create!(name: "Yuki")

      expect(User.using(:shard_1).count).to eq(1)
      expect(User.count).to eq(0)
    end

    it "should allow selecting the shard using #new" do
      u = User.using(:shard_1).new
      u.name = "Yuki"
      u.save
      ...
      u2 = User.using(:shard_1).new
      u2.name = "Yume"
      u2.save

      expect(User.using(:shard_1).all).to eq([u, u2])
    ...
    it "should ensure that the connection will be cleaned" do
      expect(ActiveRecord::Base.current_shard).to eq(:master)
      expect do
        ActiveRecord::Base.using(:shard_1) do
    ...
    it "should allow creating more than one user" do
      User.using(:shard_1).create([{ name: "Shard1 User 1" }, { name: "Shard1 User 2" }])
      User.create!(name: "User 3")

      expect(User.using(:shard_1).where(name: "Shard1 User 1").take).not_to be_nil
      expect(User.using(:shard_1).where(name: "Shard1 User 2").take).not_to be_nil
      expect(User.using(:master).where(name: "User 3").take).not_to be_nil
    end

    it "should clean #current_shard from proxy when using execute" do
      User.using(:shard_1).connection.execute("select * from users limit 1;")

      expect(User.current_shard).to eq(:master)
    end

    it "should allow scoping dynamically" do
      User.using(:shard_1).using(:master).using(:shard_1).create!(name: "oi")

      expect(User.using(:shard_1).count).to eq(1)
      expect(User.using(:shard_1).using(:master).count).to eq(0)
      expect(User.using(:master).using(:shard_1).count).to eq(1)
    end

    it "should allow find inside blocks" do
      user = User.using(:shard_2).create!(name: "User")

      User.using(:shard_2) do
        expect(User.first).to eq(user)
      end

      expect(User.using(:shard_2).where(name: "User").take).to eq(user)
    end

    it "should work with named scopes" do
      u = User.using(:shard_1).create!(name: "Admin")

      expect(User.admin.using(:shard_1).take).to eq(u)
      expect(User.using(:shard_1).admin.take).to eq(u)

      User.using(:shard_1) do
  ...
  describe ".clear_active_connections!" do
    it "should not leak connection" do
      User.using(:shard_1).create(name: "User")

      expect { ActiveRecord::Base.clear_active_connections! }
        .to change { User.using(:shard_1).connection_pool.active_connection? }

      expect(User.using(:shard_1).connection_pool.active_connection?).to be_falsey
    end
  end

  describe "#current_shard" do
    it "should store the attribute when you create or find an object" do
      user = User.using(:shard_1).create!(name: "User1")
      expect(user.current_shard).to eq(:shard_1)

      User.using(:shard_2).create!(name: "User2")
      user = User.using(:shard_2).where(name: "User2").take
      expect(user.current_shard).to eq(:shard_2)
    end

    it "should store the attribute when you find multiple instances" do
      5.times { User.using(:shard_1).create!(name: "User") }

      User.using(:shard_1).all.each do |u|
        expect(u.current_shard).to eq(:shard_1)
      end
    end

    it "should works when you find, and after that, alter that object" do
      shard_user = User.using(:shard_1).create!(name: "Shard")
      User.using(:master).create!(name: "Master")
      shard_user.name = "Shard1"
      shard_user.save

      expect(User.using(:master).first.name).to eq("Master")
      expect(User.using(:shard_1).first.name).to eq("Shard1")
    end
  end
  ...
  describe "#reload" do
    ...
    it "should work for the reload method" do
      User.using(:shard_1).where(id: user.id).update_all(name: "User2")
  ...
  describe "#touch" do
    ...
    it "updates updated_at by default" do
      User.using(:shard_1).where(id: user.id).update_all(updated_at: Time.now - 3.months)
      user.touch
    ...
    it "updates passed in attribute name" do
      User.using(:shard_1).where(id: user.id).update_all(created_at: Time.now - 3.months)
  ...
  describe "#association" do
    context "when you have a 1 x 1 relationship" do
      before(:each) do
        @supplier_shard1 = Supplier.using(:shard_1).create!(name: "Supplier Shard1")
        @supplier_shard2 = Supplier.using(:shard_2).create!(name: "Supplier Shard2")
        @account_shard1 = Account.using(:shard_1).create!(name: "Account Shard1", supplier: @supplier_shard1)
        @account_shard2 = Account.using(:shard_2).create!(name: "Account Shard2", supplier: @supplier_shard2)
      end
      ...
      it "should read correctly the relationed model" do
        new_supplier_shard1 = Supplier.using(:shard_1).create!(name: "New Supplier Shard1")
      ...
      it "should work when using #build_account" do
        supplier_shard1 = Supplier.using(:shard_1).create!(name: "Supplier Shard1")
      ...
      it "should work when using #create_account" do
        supplier_shard1 = Supplier.using(:shard_1).create!(name: "Supplier Shard1")
      ...
      it "should include models" do
        ...
        expect(Supplier.using(:shard_1).includes(:account).find(supplier_shard1.id)).to eq(supplier_shard1)
      end
    end

    context "when you have a 1 x N relationship" do
      before(:each) do
        @customer_s1 = Customer.using(:shard_1).create!(name: "Customer Shard1")
        @customer_s2 = Customer.using(:shard_2).create!(name: "Customer Shard2")
        @order_s1 = Order.using(:shard_1).create!(name: "Order Shard1", customer: @customer_s1)
        @order_s2 = Order.using(:shard_2).create!(name: "Order Shard2", customer: @customer_s2)

        @customer_s1 = Customer.using(:shard_1).where(name: "Customer Shard1").take
        Customer.using(:shard_1).create!(name: "Test")
      end
      ...
      it "should update the attribute for the order" do
        customer = Customer.using(:shard_1).create!(name: "new Customer")
      ...
      it "should works for build method" do
        order = Order.using(:shard_1).create!(name: "Order Shard1")
      ...
      describe "it should works when using" do
        before(:each) do
          @order_s1_2 = Order.using(:shard_1).create!(name: "Order Shard1 #2")
        ...
        it "empty?" do
          ...
          c = Customer.using(:shard_1).create!(name: "Customer1")
